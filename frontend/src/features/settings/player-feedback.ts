...
export function playFeedback(kind: "move" | "success" | "error" = "move") {
  ...
  try {
    ...
    oscillator.frequency.setValueAtTime(kind === "success" ? 660 : kind === "error" ? 180 : 360, now);
    ...
    gain.gain.exponentialRampToValueAtTime(0.001, now + (kind === "move" ? 0.05 : 0.18));
    oscillator.connect(gain);
    gain.connect(audioContext.destination);
    oscillator.start(now);
    oscillator.stop(now + (kind === "move" ? 0.06 : 0.2));
  } catch {
