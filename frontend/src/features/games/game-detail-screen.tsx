...
export function GameDetailScreen({
  ...
  if (game.isError) {
    return (
      <div className="border-x border-b border-[var(--line)] p-5 sm:p-8">
        <ErrorState
          title={isApiError(game.error) && game.error.status === 404 ? "Game not found" : "Game signal unavailable"}
          description={getErrorMessage(game.error)}
          actionLabel="Retry catalog link"
    ...
    <>
      <section className="grid border-x border-b border-[var(--line)] bg-[var(--surface)] lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <div className="flex min-h-[520px] flex-col justify-between p-5 sm:p-9 lg:p-12">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="font-telemetry text-[8px] text-[var(--accent)]">
              {detail.category} / {detail.gameType}
            </span>
          </div>
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {local || roomId ? (
                <a href="#game-stage" className={buttonStyles("primary")}>
                  {local ? "Start local engine" : "Open active match"}
          ...
          <dl className="grid grid-cols-2 gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-5">
            <div className="bg-[var(--background)] p-3">
              <dt className="font-telemetry text-[8px] text-[var(--muted)]">Players</dt>
              <dd className="mt-1 text-lg font-black">{detail.minPlayers}–{detail.maxPlayers}</dd>
            </div>
            <div className="bg-[var(--background)] p-3">
              <dt className="font-telemetry text-[8px] text-[var(--muted)]">Verified plays</dt>
              <dd className="mt-1 text-lg font-black">{detail.playsCount.toLocaleString()}</dd>
            </div>
            <div className="bg-[var(--background)] p-3">
              <dt className="font-telemetry text-[8px] text-[var(--muted)]">Live players</dt>
              <dd className="mt-1 text-lg font-black">{detail.onlinePlayers.toLocaleString()}</dd>
            </div>
            <div className="bg-[var(--background)] p-3">
              <dt className="font-telemetry text-[8px] text-[var(--muted)]">Cataloged</dt>
              <dd className="mt-1 text-sm font-black uppercase">
                {new Date(detail.createdAt).toLocaleDateString()}
              </dd>
            </div>
            <div className="bg-[var(--background)] p-3">
              <dt className="font-telemetry text-[8px] text-[var(--muted)]">Engine</dt>
              <dd className="mt-1 truncate text-sm font-black uppercase">{registration?.engine ?? "unregistered"}</dd>
            </div>
          </dl>
        </div>
        <div className="relative min-h-80 overflow-hidden border-t border-[var(--line)] bg-[var(--surface-strong)] lg:border-l lg:border-t-0">
          {artwork ? (
            ...
            <img src={artwork} alt={detail.name + " artwork"} className="h-full w-full object-cover" />
          ) : (
            <div className="grid h-full place-items-center">
              <span className="font-telemetry text-[9px] text-[var(--muted)]">NO VISUAL SIGNAL</span>
            </div>
          )}
      ...
      <section id="game-stage" className="scroll-mt-24 border-x border-b border-[var(--line)] bg-[var(--surface)]">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] px-5 py-4">
          <div>
            <p className="font-telemetry text-[8px] text-[var(--accent)]">[ GAME STAGE ]</p>
            <h2 className="mt-1 text-xl font-black uppercase tracking-[-0.04em]">Run interface</h2>
          </div>
      ...
      <section className="mt-20 border-x border-t border-[var(--line)] bg-[var(--surface)]">
        ...
        {achievements.isError ? (
          <div className="p-5">
            <ErrorState
              title="Achievement catalog unavailable"
              description={getErrorMessage(achievements.error)}
        ...
        {achievements.data && !relatedAchievements.length ? (
          <div className="p-5">
            <EmptyState
              title="No related objectives"
              description="The backend achievement catalog has no objective mapped to this game."
            />
          </div>
        ) : null}
      ...
      <section className="grid border-x border-t border-[var(--line)] lg:grid-cols-[1fr_360px]">
        <div className="border-b border-[var(--line)] bg-[var(--surface)] lg:border-b-0 lg:border-r">
          <header className="flex items-center justify-between border-b border-[var(--line)] px-5 py-5">
            <div>
              <p className="font-telemetry text-[8px] text-[var(--muted)]">[ SCORE CHANNEL ]</p>
              <h2 className="mt-1 text-2xl font-black uppercase tracking-[-0.04em]">Operation ranks</h2>
            </div>
          ...
          {ranking.data && !ranking.data.content.length ? (
            <div className="p-5">
              <EmptyState title="No validated scores" description="The server has not accepted a result for this operation yet." />
            </div>
          ) : null}
        ...
        <aside className="bg-[var(--background)] p-6">
          <p className="font-telemetry text-[8px] text-[var(--muted)]">[ RESULT AUTHORITY ]</p>
          <h2 className="mt-2 text-2xl font-black uppercase tracking-[-0.04em]">Verified play</h2>
          <ShieldCheck size={24} className="mt-8 text-[var(--accent)]" aria-hidden="true" />
          <p className="mt-4 text-xs leading-5 text-[var(--muted)]">
            Rankings are populated only from results accepted by the backend.
            Multiplayer clients transmit inputs while the server owns turns,
            positions, health, scoring, and progression.
          </p>
